...
const { getBanList } = require('./banSystem');
 ...
 NanoBotz.ev.on('messages.upsert', async (chatUpdate) => {
    try {
        ...
        // 🔄 Load the latest settings.json on every message
        ...
        // 🚨 Self Mode: Only Bot Owner & Bot Messages Can Be Processed
        if (!settings.publicMode && sender !== settings.botOwner && !kay.key.fromMe) {
            console.log(`⛔ Ignoring message from ${sender} (Bot is in self mode, owner-only)`);
            return;
        }

        // ✅ Auto View Status (if enabled)
        if (settings.autoStatusView && kay.key.remoteJid === 'status@broadcast') {
        ...
        const body = m.text || (m.message.conversation ? m.message.conversation : "");

        // ✅ Get the latest ban list dynamically  
        const bannedUsers = getBanList();

        // ✅ Detect the command prefix dynamically  
        const pric = /^#.¦|\\^/.test(body) ? body.match(/^#.¦|\\^/gi) : xprefix;
        const isCommand = body.startsWith(pric);

        // 🚨 If it's a command and the user is banned, block them  
        if (isCommand && bannedUsers.includes(m.sender)) {
            console.log(`⛔ Blocked command from banned user: ${m.sender}`);
